...
const ExtraHours = () => {
  ...
  const fetchExtraHourTypes = async () => {
    ...
    try {
      ...
      setError(
        "No se pudieron cargar los tipos de horas extras. Intenta recargar la página."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteType = async (id) => {
    if (
      ...
      try {
        ...
        setError(
          "No se pudo eliminar el tipo de hora extra. Por favor, inténtalo de nuevo."
        );
      }
    }
  };
  ...
  const handleSaveType = async () => {
    if (!currentType.type || !currentType.multiplier) {
      setError("El Tipo y el Multiplicador no pueden estar vacíos.");
      return;
    }
    if (isNaN(parseFloat(currentType.multiplier))) {
      setError("El Multiplicador debe ser un número válido.");
      return;
    }
    if (parseFloat(currentType.multiplier) <= 0) {
      setError("El Multiplicador debe ser un valor positivo.");
      return;
    }
    ...
    try {
      ...
      console.error(
        "Error saving extra hour type:",
        err.response?.data || err.message
      );

      setError(
        err.response?.data ||
          "No se pudo guardar el tipo de hora extra. Por favor, inténtalo de nuevo."
      );
    }
  };
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        {/* Modal para crear/editar tipo de hora extra */}
        {isModalOpen && (
